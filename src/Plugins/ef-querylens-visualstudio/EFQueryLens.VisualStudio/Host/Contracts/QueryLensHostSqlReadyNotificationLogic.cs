...
internal static class QueryLensHostSqlReadyNotificationLogic
{
    ...
    public static bool ShouldNotify(QueryLensHostSqlReadyNotification? payload)
        => payload is not null && payload.CommandCount > 0;

    public static bool ShouldShow(QueryLensHostSqlReadyNotification payload, bool enabled, long nowMs)
    {
    ...
    public static string BuildToastMessage(QueryLensHostSqlReadyNotification payload)
    {
    ...
    internal static void ResetDedupeForTests() => RecentNotifications.Clear();
}
