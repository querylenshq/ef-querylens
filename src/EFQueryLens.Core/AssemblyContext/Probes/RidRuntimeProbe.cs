...
/// <summary>
/// Resolves platform-native runtime assemblies from the
/// <c>runtimes/{rid}/lib/{tfm}/</c> layout inside the output directory.
/// </summary>
internal sealed class RidRuntimeProbe : AssemblyProbe
{
    private readonly string _assemblyDirectory;
    private readonly string[] _runtimeRidProbeOrder;

    internal RidRuntimeProbe(string assemblyDirectory)
    {
    ...
    internal override string? TryResolve(AssemblyName assemblyName) =>
        string.IsNullOrWhiteSpace(assemblyName.Name) ? null : TryResolve(assemblyName.Name);

    /// <summary>
    /// Resolves by simple name only — used by <c>NormalizeAssemblyPathForLoad</c>.
    /// </summary>
    internal string? TryResolve(string assemblySimpleName)
    {
        ...
        try
        {
            foreach (var path in Directory.EnumerateFiles(runtimesRoot, fileName, SearchOption.AllDirectories))
            {
                ...
                candidates.Add((path, GetRidScore(rid), GetTfmScore(tfm)));
            }
        }
        catch
        {
    ...
    private static int GetTfmScore(string? tfm)
    {
        ...
        if (tfm.StartsWith("net", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (int.TryParse(versionText, out var majorOnly))
                return majorOnly <= 10 ? 2000 + (majorOnly * 10) : 500 + majorOnly;
