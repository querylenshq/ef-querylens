...
// Usage: node tsp-review-packet.cjs <impldoc-path> [--diff-from <ref>] [--output <path>]
...
function parseArgs(argv) {
  ...
  if (!args.impldocPath) {
    console.error("Usage: node tsp-review-packet.cjs <impldoc-path> [--diff-from <ref>] [--output <path>] [--quiet]");
    process.exit(2);
  }
  return args;
}
